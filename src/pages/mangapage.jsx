...
export default function MangaPage(){
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const response = await fetch(`https://api.mangabridge.com/manga/${params.name}`, {
                    credentials: "include",
        ...
        async function check(Mmanga) {
            try {
                const response = await fetch(`https://api.mangabridge.com/manga/check/${userInfo.id}&${Mmanga._id}`, {
                    headers: { 'Content-Type': 'application/json' },
                });
        ...
        async function chapterGetter(mangaId) {
            try {
                const response = await fetch(`https://api.mangabridge.com/chapter/relevant/${mangaId}`, {
                    credentials: "include",
                    headers: { 'Content-Type': 'application/json' },
                });
        ...
        async function CommentsGetter(mangaId) {
            try {
                const response = await fetch(`https://api.mangabridge.com/manga/comments/${mangaId}`, {
                    headers: { 'Content-Type': 'application/json' },
                });
    ...
    function Kaydet(userId,mangaId){
        fetch(`https://api.mangabridge.com/manga/save/${userId}&${mangaId}`, {
            headers: { 'Content-Type': 'application/json' },
    ...
    function SubmitComment(ev){
        ...
        ev.preventDefault()
        fetch("https://api.mangabridge.com/manga/comments/add",{ 
            method:"POST",
            body: JSON.stringify({content:content,user:userInfo.id,manga:Mmanga._id}),
            headers: {'Content-Type':'application/json'},
            credentials: 'include',
        }).then(response => response.json()).then(data => setComments(data)).then(setContent(""))
    ...
    function deletecomment(id){
        fetch("https://api.mangabridge.com/manga/comments/delete",{ 
            method:"POST",
                ...
                <div className="grid gap-10 lg:grid-cols-9 grid-cols-1">
                    <div className="lg:col-span-2 lg:w-auto w-screen">
                        <div className=" bg-white shadow-xl bg-opacity-5 backdrop-blur-sm text-3xl break-words fredoka p-4 rounded-3xl drop-shadow-lg">
                            <img src={"https://api.mangabridge.com/Collection/"+Mmanga.image} alt="" className="rounded-xl mx-auto"/>
                            ...
                            <div className=" ">
                                {reader &&
                                    <div className="grid lg:grid-cols-4 grid-cols-2 gap-6">
                                        {Fansub && Fansub.map((Fansubs, indexs) => (
                                            <div key={indexs} className="font-bold text-center mt-6 rounded-2xl p-2 h-96 overflow-y-scroll no-scrollbar" style={myStyle}>
                                                ...
                                                {Chapter.filter((chapter) => chapter.fansub === Fansubs).map(
                                                    (chapter, index) => (
                                                        <Link to={`/manga/read/${(chapter.url).replace(/\/+/g,"*")}`} key={index}>
                                                            <div className="text-2xl w-full p-2 my-2 bg-[#161616] shadow-xl rounded-xl"  >{chapter.number}. Bölüm</div>
                                                        </Link>
                                                    )
                                                )}
                                            </div>
