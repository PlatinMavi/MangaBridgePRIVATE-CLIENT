...
export default function SearchBar() {
  ...
  const search = async () => {
    try {
      ...
      const response = await fetch(`http://16.171.37.129:4000/manga/search?key=${key}`, {
        credentials: 'include',
            ...
            searchResults.map((manga) => (
              <Link to={"/manga/"+manga.browser} >
                <div className="flex w-64 break-words my-1 gap-x-2">
                    <img src={"http://16.171.37.129:4000/Collection/"+manga.image} alt="" className="w-16 h-24 rounded-lg" />
